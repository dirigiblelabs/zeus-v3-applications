...
.factory('$messageHub', [function(){
	...
	var message = function(evtName, data){
		messageHub.post({data: data}, 'zeus.Applications.Variables.' + evtName);
	};
	...
	return {
		...
		onEntityRefresh: function(callback) {
			on('zeus.Applications.Variables.refresh', callback);
		},
...
.controller('PageController', function ($scope, $http, $messageHub) {
	...
	var api = '/services/v3/js/zeus-applications/api/Applications/Variables.js';
	...
	$scope.loadPage = function(pageNumber) {
		...
		.success(function(data) {
			...
			$http.get(api + '?Application=' + $scope.masterEntityId + '&$offset=' + ((pageNumber - 1) * $scope.dataLimit) + '&$limit=' + $scope.dataLimit)
			.success(function(data) {
				$scope.data = data;
			});
		});
